...
@export var skeleton : MixamoSkeleton
...
func set_animations():
	if full_body_mode:
		if legs_animator.current_animation != model.current_move.animation:
			set_legs_animation(model.current_move.animation)
			set_torso_animation(model.current_move.animation)
			synchronize_if_needed()
	else:
		if legs_animator.current_animation != model.legs.current_legs_move.animation:
			set_legs_animation(model.legs.current_legs_move.animation)
		if torso_animator.current_animation != model.current_move.animation:
			set_torso_animation(model.current_move.animation)
